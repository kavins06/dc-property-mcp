...
-- OTR publishes the cookie-bootstrap route, not its post-check redirect:
  ...
  with matching as (
    ...
    select
      ...
      jsonb_build_object(
        ...
        'access',
          'Public real-property search; first-party cookies are required.'
      ) source
    from matching
    group by property
  )
...
comment on function api_v1.get_source_evidence(text[]) is
  'Validates source references, preserves internal evidence, and groups tax and assessment references into human-facing MyTax verification sources using the official cookie-bootstrap route.';
