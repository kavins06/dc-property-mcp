...
Authenticated conversational access to Quoin's existing property MCP. The app uses Gemini on Google Vertex AI through the Vercel AI SDK, discovers the existing read-only MCP tools at runtime, keeps workspaces in browser IndexedDB, and exports conversations as PDF, Excel, Word, or Markdown. It does not add or modify a database.

## Local setup

For this workstation, the app can use the active WorkOS CLI environment and Google Application Default Credentials without copying either secret into the repository:
...
- `GOOGLE_VERTEX_PROJECT`, `GOOGLE_VERTEX_LOCATION`, and `GEMINI_MODEL`; local development uses Google Application Default Credentials (`gcloud auth application-default login`)
...
Also set `GCP_PROJECT_NUMBER`, `GCP_SERVICE_ACCOUNT_EMAIL`, `GCP_WORKLOAD_IDENTITY_POOL_ID`, and `GCP_WORKLOAD_IDENTITY_POOL_PROVIDER_ID`. The app exchanges Vercel OIDC tokens through Google Workload Identity Federation, so no long-lived Google service-account key is stored.
